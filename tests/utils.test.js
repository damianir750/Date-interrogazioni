...
describe("validateStudent", () => {
  ...
  it("rejects empty name or whitespace only", () => {
    expect(validateStudent({ name: "", subject: "It" }).length).toBeGreaterThan(
      0,
    );
    expect(
      validateStudent({ name: "   ", subject: "It" }).length,
    ).toBeGreaterThan(0);
  });
...
describe("validateSubject", () => {
  ...
  it("rejects colors without # prefix", () => {
    expect(
      validateSubject({ name: "T", color: "ABCDEF" }).length,
    ).toBeGreaterThan(0);
  });

  it("rejects 3-digit hex if 6 is required (usually better)", () => {
    expect(
      validateSubject({ name: "T", color: "#FFF" }).length,
    ).toBeGreaterThan(0);
  });
});
