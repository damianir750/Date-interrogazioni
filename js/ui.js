...
const safeColor = (color) =>
  /^#[0-9A-Fa-f]{6}$/.test(color) ? color : "#6b7280";
...
export const ui = {
  ...
  async confirmDialog(message) {
    return new Promise((resolve) => {
      ...
      overlay.className =
        "fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 transition-opacity duration-200";
      ...
      dialog.className =
        "bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-sm w-full p-6 transform scale-95 transition-transform duration-200 border border-gray-100 dark:border-gray-700";
      dialog.innerHTML = `
  ...
  updateStats(students) {
    ...
    const alerts = students.filter(
      (s) => utils.daysSince(s.last_interrogation) > 14,
    ).length;
  ...
  renderSubjectsList(subjects, students) {
    ...
    subjects.forEach((subject) => {
      ...
      div.className =
        "flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition shadow-sm";
      ...
      colorDot.className =
        "w-8 h-8 rounded-full shadow-inner border border-black/5";
      ...
      if (count > 0) {
        ...
        deleteBtn.addEventListener("click", () =>
          app.deleteSubject(subject.name),
        );
      }
  ...
  renderSkeletons(subjects = [], studentCounts = {}) {
    ...
    subjectsToRender.forEach((subject, i) => {
      const count =
        studentCounts && studentCounts[subject.name]
          ? studentCounts[subject.name]
          : 1;
      ...
      cardInner.className =
        "bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 h-full border border-gray-100 dark:border-gray-700 flex flex-col justify-start";
      ...
      for (let j = 0; j < count; j++) {
        ...
        ["🎓 0", "00/00/0000", "00g"].forEach((text, k) => {
          ...
          span.className =
            "text-transparent text-xs px-2 py-0.5 rounded-full skeleton";
        ...
        actions.className =
          "flex items-start flex-shrink-0 gap-0.5 -mt-1 opacity-50";
        for (let k = 0; k < 5; k++) {
  ...
  render(students, subjectColors, searchTerm) {
    ...
    Object.keys(bySubject).forEach((subject) => {
      bySubject[subject].sort((a, b) => {
        ...
        return (
          utils.daysSince(b.last_interrogation) -
          utils.daysSince(a.last_interrogation)
        );
      });
    });
    ...
    Object.keys(bySubject)
      ...
      .forEach((subject, groupIndex) => {
        ...
        const urgentCount = subjectStudents.filter(
          (s) => utils.daysSince(s.last_interrogation) > 30,
        ).length;
        ...
        title.className =
          "text-xl sm:text-2xl font-semibold flex items-center gap-2";
        ...
        badge.className =
          "text-sm font-medium px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300";
        ...
        if (urgentCount > 0) {
          ...
          alert.className =
            "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-xs px-3 py-1 rounded-lg mb-3 flex items-center gap-1";
        ...
        subjectStudents.forEach((s, i) => {
          const li = this.createStudentElement(
            s,
            i,
            color,
            lightColor,
  ...
  createStudentElement(
    ...
    if (isInitialRender) {
      li.className =
        "flex justify-between items-center p-3 rounded-lg shadow-sm slide-in dark:text-gray-200";
      li.style.animationDelay = `${groupIndex * 0.1 + i * 0.05}s`;
    } else {
      li.className =
        "flex justify-between items-center p-3 rounded-lg shadow-sm dark:text-gray-200 transition-all";
    }
    ...
    if (days === -1) {
      dateInfo.className =
        "text-[10px] sm:text-xs text-amber-700 dark:text-amber-500 font-semibold whitespace-nowrap";
      dateInfo.textContent = "📅 MANCANTE";
    } else {
      dateInfo.className =
        "text-[10px] sm:text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap";
    ...
    statusBadge.className =
      "text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-bold whitespace-nowrap";
    if (days === -1) {
    ...
    actionsDiv.appendChild(
      createBtn(
        ...
        "Segna come interrogato",
        () => app.registerInterrogation(s.id, gradesCount),
      ),
    );
    ...
    if (days !== -1) {
      actionsDiv.appendChild(
        createBtn(
          ...
          "Aggiungi voto (+1)",
          () => app.incrementGrade(s.id, gradesCount),
        ),
      );
    }
    ...
    actionsDiv.appendChild(
      createBtn(
        ...
        () => app.updateStudentGrades(s.id, gradesCount),
      ),
    );
    actionsDiv.appendChild(
      createBtn(
        ...
        "Modifica nome",
        () => app.updateStudentName(s.id, s.name),
      ),
    );
    actionsDiv.appendChild(
      createBtn("trash-2", "text-red-500 hover:text-red-700", "Elimina", () =>
        app.deleteStudent(s.id),
      ),
    );
